...
private void PrintMain (PrintWriter out) // main index.html
{
   ...
   out.println("					ErrorMessage += \"<pre>\n</pre>\" + NumErrors + \") Major should be a valid name, not a number. For example: CS, SWE, IT, etc.  \";");
   out.println("				}");
   out.println("				if (Major.value == '')");
   out.println("                            {");
   out.println("				NumErrors++;");
   out.println("			ErrorMessage += '<pre>\n</pre>' + NumErrors + ') Missing major.  ';");
   out.println("			}");
   out.println("				var Year = document.UserInfo.Year;");
   out.println("		if (!IsNum(Year.value)) { ");
   out.println("				NumErrors++;");
   out.println("					ErrorMessage += \"<pre>\n</pre>\" + NumErrors + \") Year should be an integer (ex: 1 = Freshman, 2 = Sophomore, etc.)  \";");
   out.println("				}");
   out.println("				if (Year.value == '')");
   out.println("                                {");
   out.println("					NumErrors++;");
   out.println("					ErrorMessage += \"<pre>\n</pre>\" + NumErrors + \") Missing year.  \";");
   out.println("				}");
